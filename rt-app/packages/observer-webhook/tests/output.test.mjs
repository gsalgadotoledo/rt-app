import test from 'node:test';import assert from 'node:assert/strict';import {WebhookOutput} from '../dist/index.js';
...
test('provider payload, abort propagation and rejection handling without network calls',async()=>{
 ...
 const handler=new WebhookOutput('custom','https://logs.example.test',{},transport),signal=new AbortController().signal;
 await handler.write(event,signal);assert.equal(calls[0].signal,signal);assert.equal(calls[0].redirect,'error');assert.equal(JSON.parse(calls[0].body).requestId,'request-1');
