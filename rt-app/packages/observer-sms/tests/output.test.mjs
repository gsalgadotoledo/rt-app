import test from 'node:test';import assert from 'node:assert/strict';import {SmsOutput} from '../dist/index.js';
test('AWS adapter forwards sanitized events and cancellation to the configured client',async()=>{
 ...
 await new SmsOutput('+15555550123',client).write({id:'test',at:new Date().toISOString(),level:'error',kind:'log',category:'payments',source:'app',message:'Declined',data:{}},signal);
