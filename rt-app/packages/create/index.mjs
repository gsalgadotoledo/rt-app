...
export async function copyStarter(source,destination, selectedRoots){
 const roots=selectedRoots??['apps','packages','rt-app','infra','.github','.gitignore','.gitlab-ci.yml','package.json','package-lock.json','main.js','main.d.ts','modules.json','rt-app.settings.json','README.md','CLAUDE.md','AGENTS.md'];
...
export async function starterRoot(){const source=resolve(packageRoot,'../../..');try{await access(join(source,'rt-app.settings.json'));return source;}catch{}const bundled=join(packageRoot,'starter');await access(join(bundled,'package.json'));return bundled;}
...
export async function createProject({workspace,name,templateId='fullstack',backendId='node-ts',install=true,source,env=process.env,onLog=()=>{}}){
 ...
 try{
  ...
  const generator=await import(pathToFileURL(join(target,'rt-app/cli/bin/create-crud.mjs')));await rm(join(target,'packages'),{recursive:true,force:true});await mkdir(join(target,'packages'));for(const [p,body] of Object.entries(generator.registries([])))await writeFile(join(target,p),body);
  ...
  if(spec.kind==='electron'){
   ...
   await save('apps/desktop/package.json',{name:'@app/desktop',private:true,type:'module',scripts:{dev:'electron .'},main:'main.mjs',dependencies:{electron:'44.4.4','@gsalgadotoledo/rt-app-config':'*'}});
  ...
  if(backendId!=='node-ts'){
   ...
   for(const script of ['setup','setup:terminal','lambda:build'])pkg.scripts[script]='node rt-app/scripts/native-deploy-pending.mjs';
