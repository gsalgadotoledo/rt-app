...
export class EmailOutput implements ObserverOutputHandler {
  ...
  constructor(
    private from: string,
    private to: string,
    private client = new SESv2Client({ maxAttempts: 1 }),
  ) {
    if (!from.includes("@") || !to.includes("@"))
      throw new Error("Observer email requires valid from/to addresses");
  }
  async write(event: ObserverEvent, signal?: AbortSignal) {
    await this.client.send(
      new SendEmailCommand({
        ...
        Content: {
          Simple: {
            Subject: { Data: `[${event.level}] ${event.source}` },
            Body: { Text: { Data: JSON.stringify(event, null, 2) } },
          },
        },
      }),
      { abortSignal: signal },
...
export class LocalEmailOutput implements ObserverOutputHandler {
  ...
  constructor(
    ...
    if (process.env.NODE_ENV === "production")
      throw new Error("Local observer email is disabled in production");
  ...
  async write(event: ObserverEvent) {
    await this.transport.sendMail({
      from: this.from,
      to: this.to,
      subject: `[${event.level}] ${event.source}`,
