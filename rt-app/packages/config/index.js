...
/** AWS credentials alone never turn a development process into a cloud runtime. */
export function runtimeConfig(settings, env = process.env) {
  ...
  if (explicit && !['local','aws'].includes(explicit)) throw new Error('Invalid RT_APP_TARGET');
  if (lambda && (explicit === 'local' || (env.RT_APP_MODE && env.RT_APP_MODE !== 'aws'))) throw new Error('Lambda cannot use local data adapters');
  const target = lambda ? 'aws' : (explicit ?? (env.RT_APP_MODE === 'aws' ? 'aws' : 'local'));
  const profile = settings?.runtime?.[target];
  ...
  if (target === 'local' && !['json','memory','dynamodb-local'].includes(mode)) throw new Error('Unsupported local adapter');
  return {target,mode};
}
