...
export async function openDataApplication() {
  ...
  return createApplication({
    store: mode === "json" ? new JsonStore(file) : await localDynamo(),
    localAdminAccess: true,
    mailer: new LocalMailbox(),
    secret: mode === "json" ? await localSecret(file) : randomBytes(48).toString("hex"),
